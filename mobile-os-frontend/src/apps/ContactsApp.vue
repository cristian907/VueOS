<template>
  <div class="w-full h-full bg-neutral-100 dark:bg-slate-900 flex flex-col overflow-hidden transition-colors">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-2">
      ...
      >
        <div class="w-12 h-12 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center text-gray-600 dark:text-gray-300 font-bold text-lg shadow-inner">
          {{ contact.avatar }}
        </div>
        <div class="flex-1">
          <h3 class="font-semibold text-lg text-black dark:text-white">{{ contact.name }}</h3>
          <p class="text-gray-500 dark:text-gray-400 text-sm">ID: {{ contact.id }}</p>
        </div>
        <!-- Delete Button -->
        <button @click="contactsStore.removeContact(contact.id)" class="text-red-400 dark:text-red-500 p-2">
      ...
      <div class="bg-white dark:bg-slate-800 w-full sm:rounded-3xl rounded-t-3xl p-6 shadow-2xl animate-slide-up sm:animate-fade-in transition-colors">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input v-model="newName" type="text" class="w-full bg-gray-100 dark:bg-slate-700 text-black dark:text-white p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-lg" placeholder="Ej. Juan Pérez">
          </div>
          <div>
            <label class="block text-xs uppercase text-gray-500 dark:text-gray-400 font-bold mb-1">ID Móvil / Número</label>
            <input v-model="newId" type="text" class="w-full bg-gray-100 dark:bg-slate-700 text-black dark:text-white p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-lg" placeholder="Ej. j-1234">
          </div>
